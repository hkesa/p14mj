...
function renderStatsMatrix() {
    ...
    // V1.34 "雞糊王" and "三番王"
    const chickenHandStats = {};
    const threeFanStats = {};
    PLAYERS.forEach(p => {
        chickenHandStats[p] = { wins: 0, total: 0 };
        threeFanStats[p] = { wins: 0, total: 0 };
    });

    chronologicalGames.forEach(game => {
        const isThreeFan = game.threeFanMin === true;

        PLAYERS.forEach(p => {
            if (game.players[p] && game.players[p].seat) {
                // Participated in this game
                if (isThreeFan) {
                    threeFanStats[p].total++;
                    if (game.players[p].score >= 0) {
                        threeFanStats[p].wins++;
                    }
                } else {
                    chickenHandStats[p].total++;
                    if (game.players[p].score >= 0) {
                        chickenHandStats[p].wins++;
                    }
                }
            }
        });
    ...
    const chickenHandKingRanking = PLAYERS.map(p => {
        const st = chickenHandStats[p];
        const pct = st.total > 0 ? (st.wins / st.total * 100).toFixed(1) : 0;
        return { p, val: st.wins, total: st.total, pct: pct };
    }).filter(x => x.total > 0).sort((a, b) => b.val - a.val).slice(0, 4);

    const threeFanKingRanking = PLAYERS.map(p => {
        const st = threeFanStats[p];
        const pct = st.total > 0 ? (st.wins / st.total * 100).toFixed(1) : 0;
        return { p, val: st.wins, total: st.total, pct: pct };
    }).filter(x => x.total > 0).sort((a, b) => b.val - a.val).slice(0, 4);

    const formatKingRank = (list) => {
        return list.map((item, index) => {
            ...
            <div class="stats-item">
                <span>${prefix}${item.p}</span>
                <span>
                    <span class="num-pos">勝${item.val}局</span>
                    <span style="font-size:0.8em; color:#666; margin-left:5px;">(${item.pct}%)</span>
                </span>
            </div>`;
        }).join('');
    };
    ...
    additionalContainer.innerHTML = `
        <!-- V1.34 Chicken Hand King -->
        <div class="streak-card">
            <div class="stats-grid">
                <div>
                    <strong>雞糊王 (總局數${chronologicalGames.filter(g => !g.threeFanMin).length})：</strong>
                    ${chickenHandKingRanking.length > 0 ? formatKingRank(chickenHandKingRanking) : '<div class="stats-item num-zero">暫無足夠組合</div>'}
                </div>
            </div>
        </div>

        <!-- V1.34 Three Fan King -->
        <div class="streak-card">
            <div class="stats-grid">
                <div>
                    <strong>三番王 (總局數${chronologicalGames.filter(g => g.threeFanMin).length})：</strong>
                    ${threeFanKingRanking.length > 0 ? formatKingRank(threeFanKingRanking) : '<div class="stats-item num-zero">暫無足夠組合</div>'}
                </div>
            </div>
        </div>
    ...
    btn.addEventListener('click', () => {
        ...
        const interval = setInterval(() => {
            ...
            if (rolls > 15) {
                ...
                // Explicitly generate 3 independent rolls 1-6
                // Using crypto for better distribution if available, fallback to Math.random
                const getRandomRoll = () => {
                ...
                const v1 = getRandomRoll();
                const v2 = getRandomRoll();
                const v3 = getRandomRoll();
